...
class ActionCostTableTest {
    private val paletteCosts =
        mapOf(
            ...
            // Task 4.3/4.4 extensions (phase-4 report): the deterministic replan
            // hand-off (the §6.1 step-8 transition) and the plan-space escalation.
            "replan" to 0.05,
            "exhaustPlanSpace" to 0.00,
    ...
    fun `every action declares a cost matching the plan table`() {
        assertEquals(
            ...
            "annotated actions must match the §6 table exactly (resume = Phase-5 HITL continuation; replan/exhaustPlanSpace = the Task-4.3/4.4 lanes)",
        )
